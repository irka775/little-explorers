...
def cache_checkout_data(request):
    try:
        ...
        return HttpResponse(content="Sorry, your payment cannot be processed right now", status=400)
...
def checkout(request):
    ...
    intent = None  # Inițializăm pentru a evita UnboundLocalError
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            pid = request.POST.get('client_secret', '').split('_secret')[0] if 'client_secret' in request.POST else None
    ...
    context = {
        ...
        'client_secret': intent.client_secret if intent else '',  # Evităm eroarea UnboundLocalError
    }
